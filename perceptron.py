...
for clf in classifiers:
    ...
    with open(f'./results/{clf}_results.csv', 'w') as file:
        file.write('module,attributes,technique,metric,score\n')
    ...
    for mod in modules:
        ...
        for atbt in attributes:
            ...
            model = MLPClassifier(hidden_layer_sizes=(1000,1000), max_iter=1000, random_state=0)
            ...
            print(f'\n[{clf.upper()}] {mod.upper()} : {atbt.upper()} (NONE)')
            print('--------------------')
            ...
            #     file.write(f'{mod},{atbt},none,accuracy,{accuracy}\n')
            #     file.write(f'{mod},{atbt},none,fscore,{fscore}\n')
            #     file.write(f'{mod},{atbt},none,precision,{precision}\n')
            #     file.write(f'{mod},{atbt},none,recall,{recall}\n')

        mod_end = time.time()
        print(f'\n\t==> {clf.upper()}-{mod.upper()} TIME: {round(mod_end - mod_start, 2)} sec\n')

    clf_end = time.time()
    print(f'\n\t==> ALL {clf.upper()}: {round(clf_end - clf_start, 2)} sec\n')
