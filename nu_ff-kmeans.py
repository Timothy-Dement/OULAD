...
for mod in modules:
    if os.path.exists(f'./nu_results/{mod}_ff-kmeans_results.csv'):
        os.remove(f'./nu_results/{mod}_ff-kmeans_results.csv')
        ...
        for clf in classifiers:
            ...
            for cl in pd.Series(cluster_labels).unique():
                ...
                # If non-homogenous, train a classifer on the cluster
                    ...
                    model.fit(Xcl_train, ycl_train, epochs=10, batch_size=10)
            ...
            if not os.path.exists(f'./nu_results/{mod}_ff-kmeans_results.csv'):
                with open(f'./nu_results/{mod}_ff-kmeans_results.csv', 'w') as file:
                    file.write(f'module,attributes,classifier,technique,metric,score\n')
            
            with open(f'./nu_results/{mod}_ff-kmeans_results.csv', 'a') as file:
