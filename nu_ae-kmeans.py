...
for mod in modules:
    if os.path.exists(f'./nu_results/{mod}_ae-kmeans_results.csv'):
        os.remove(f'./nu_results/{mod}_ae-kmeans_results.csv')
        ...
        for clf in classifiers:
            ...
            for cl in pd.Series(cluster_labels).unique():
                ...
                # If non-homogenous, train a classifer on the cluster
                    ...
                    num_enc_feats = int(round(Xcl_train.shape[0] ** 0.5))

                    input_dim = Input(shape = (Xcl_train.shape[1],))
                    ...
                    decoded3 = Dense(Xcl_train.shape[1], activation = 'relu')(decoded2)
                    
                    autoencoder = Model(inputs = input_dim, outputs = decoded3)

                    autoencoder.compile(optimizer = 'adam', loss = 'binary_crossentropy',  metrics=['accuracy'])

                    autoencoder.fit(Xcl_train, Xcl_train, epochs = 10, batch_size = 10, shuffle = False, validation_data = (Xcl_test, Xcl_test))
                    
                    encoder = Model(inputs = input_dim, outputs = encoded3)
                    encoded_input = Input(shape = (num_enc_feats, ))
                    
                    encoded_train = pd.DataFrame(encoder.predict(Xcl_train))
            ...
            for cl in pd.Series(test_clusters).unique():
                ...
                if cl_mod == 0:
                    ...
                    encoded_test = pd.DataFrame(cluster_encoders[cl].predict(Xcl_test))
                    encoded_test = encoded_test.add_prefix('feature_')

                    ycl_hat = cluster_models[cl].predict(encoded_test)
            ...
            if not os.path.exists(f'./nu_results/{mod}_ae-kmeans_results.csv'):
                with open(f'./nu_results/{mod}_ae-kmeans_results.csv', 'w') as file:
                    file.write(f'module,attributes,classifier,technique,metric,score\n')
            
            with open(f'./nu_results/{mod}_ae-kmeans_results.csv', 'a') as file:
