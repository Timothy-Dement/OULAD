...
for mod in modules:
    if os.path.exists(f'./nu_results/{mod}_smote+pca_results.csv'):
        os.remove(f'./nu_results/{mod}_smote+pca_results.csv')
...
for mod in modules:
    ...
    for atbt in attributes:
        ...
        for clf in classifiers:
            ...
            print('\n[ {0} : {1} : {2} : SMOTE+PCA ]'.format(mod.upper(), atbt.upper(), clf.upper()))
            print('+----------------------+')
            ...
            if not os.path.exists(f'./nu_results/{mod}_smote+pca_results.csv'):
                with open(f'./nu_results/{mod}_smote+pca_results.csv', 'w') as file:
                    file.write(f'module,attributes,classifier,technique,metric,score\n')
            
            with open(f'./nu_results/{mod}_smote+pca_results.csv', 'a') as file:
