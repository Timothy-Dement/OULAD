...
for df in [dt_base, knn_base, nb_base, rf_base, svm_base]:
    df.rename(index=str, columns={'sample': 'extraction'}, inplace=True)
    df['extraction'] = 'none'

dt_df = dt_base.append(dt_pca)
knn_df = knn_base.append(knn_pca)
nb_df = nb_base.append(nb_pca)
rf_df = rf_base.append(rf_pca)
svm_df = svm_base.append(svm_pca)
...
for mod in ['aaa', 'bbb', 'ccc', 'ddd', 'eee', 'fff', 'ggg', 'zzz']:
    ...
    for metric_df in [acc_df, fscore_df, prec_df, rec_df]:
        ...
        mod_plot = sns.barplot(ax=ax, x='id', y='score', hue='extraction', hue_order=['none', 'pca'], data=metric_df)
        ...
        mod_plot.figure.savefig(f'./charts/pca-{mod}-{metric.lower()}.png')
