...
for clf in classifiers:
    ...
    with open(f'./results/{clf}_pca_results.csv', 'w') as file:
        file.write('module,attributes,extraction,metric,score\n')
    ...
    for mod in modules:
        ...
        for atbt in attributes:
            ...
            print(f'\n[{clf.upper()}] {mod.upper()} : {atbt.upper()} (PCA)')
            print('--------------------')
            ...
            with open(f'./results/{clf}_pca_results.csv', 'a') as file:
                file.write(f'{mod},{atbt},pca,accuracy,{accuracy}\n')
                file.write(f'{mod},{atbt},pca,fscore,{fscore}\n')
                file.write(f'{mod},{atbt},pca,precision,{precision}\n')
                file.write(f'{mod},{atbt},pca,recall,{recall}\n')

        mod_end = time.time()

        print(f'\n\t==> {mod.upper()} TIME: {round(mod_end - mod_start, 2)} sec\n')
